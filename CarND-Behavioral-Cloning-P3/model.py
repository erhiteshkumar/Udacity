...
from keras.layers import Flatten, Dense, Lambda, SpatialDropout2D, Dropout
from keras.layers.convolutional import Convolution2D
from keras.layers.pooling import MaxPooling2D
from keras.layers import Cropping2D
from keras.optimizers import Adam

from utils import randomly_drop_low_steering_data, training_image_generator
...
## Opening any image to obatin input size for model 
image_filename = training_data['center'].iloc[262].split('/')[-1]
img = read_image(os.path.join(DATA_PATH + '/IMG', image_filename.strip()))
...
model.fit_generator(training_image_generator(training_data, BATCH_SIZE, DATA_PATH), samples_per_epoch=samples_per_epoch, nb_epoch=EPOCHS, validation_data=get_images(validation_data, DATA_PATH), nb_val_samples=len(validation_data))
